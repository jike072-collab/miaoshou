...
async function loadAll() {
  ...
  state.automationLogs = automationLogs.items || [];
  state.automationFailures = automationFailures.items || [];
...
const automationCounterLabels = {
  foundCount: "搜索到数量",
  savedCandidates: "保存候选数量",
  duplicateSkipped: "重复跳过数量",
  riskBlocked: "风险阻断数量",
  lowPrioritySkipped: "低优先级跳过数量",
  titleCleaned: "标题清洗数量",
  imageReady: "图片准备数量",
  collectedToBox: "已进入采集箱数量",
  failed: "失败数量",
};

function renderAutomationConsole(data = state.automation) {
  const wrap = $("#automation-console");
  if (!wrap) return;
  const counters = data.counters || {};
  const totalProgress = Math.max(0, Math.min(100, Number(data.totalProgress || 0)));
  const itemProgress = Math.max(0, Math.min(100, Number(data.itemProgress || 0)));
  const status = data.status || "idle";
  const statusClass = ["failed", "waiting_for_manual", "stopped"].includes(status) ? "check-warn" : "check-ok";
  const counterCards = Object.entries(automationCounterLabels).map(([key, label]) => {
    const danger = key === "failed" && Number(counters[key] || 0) > 0;
    return `<article class="${danger ? "precheck-danger" : ""}"><span>${esc(label)}</span><strong>${Number(counters[key] || 0)}</strong></article>`;
  }).join("");
  const steps = (data.steps || []).map((step) => `<span class="automation-step ${step.status || ""}"><i>${Number(step.progress || 0)}%</i>${esc(step.label || step.key || "")}</span>`).join("");
  wrap.innerHTML = `
    <div class="automation-status-grid">
      <article><span>当前 run_id</span><strong>${esc(data.runId || "-")}</strong></article>
      <article><span>当前关键词</span><strong>${esc(data.currentKeyword || "-")}</strong></article>
      <article><span>当前页数</span><strong>${Number(data.currentPage || 0)}</strong></article>
      <article><span>当前商品</span><strong>${esc(data.currentProduct || "-")}</strong></article>
      <article><span>当前步骤</span><strong class="${statusClass}">${esc(data.currentStep || automationStatusText[status] || "-")}</strong></article>
      <article><span>状态</span><strong class="${statusClass}">${esc(automationStatusText[status] || status)}</strong></article>
    </div>
    <div class="automation-progress-grid">
      <div><span>总进度</span><strong>${totalProgress}%</strong><i><b style="width:${totalProgress}%"></b></i></div>
      <div><span>当前商品进度</span><strong>${itemProgress}%</strong><i><b style="width:${itemProgress}%"></b></i></div>
    </div>
    <div class="automation-step-strip">${steps}</div>
    <div class="automation-counter-grid">${counterCards}</div>
    ${data.error ? `<p class="environment-alert">${esc(data.error)}</p>` : ""}
  `;
  const active = Boolean(data.active || ["queued", "running"].includes(status));
  const waiting = status === "waiting_for_manual";
  $("#automation-start").disabled = active;
  $("#automation-pause").disabled = !active;
  $("#automation-resume").disabled = !waiting && status !== "stopped";
  $("#automation-stop").disabled = !active && !waiting;
}

function automationFailureCard(item) {
  return `<details class="failure-card automation-failure-card">
    <summary>
      <div><strong>${esc(item.type || item.label || "失败任务")}</strong><span>${esc([item.product, item.market, item.shop].filter(Boolean).join(" · ") || item.batch || item.productId || "未关联商品")}</span></div>
    ...
    <div class="failure-actions">
      ${(item.actions || []).includes("retry") ? `<button class="table-action" data-failure-action="retry" data-automation-failure-action="retry" data-failure-source="${esc(item.source)}" data-failure-id="${esc(item.id)}" type="button">重试</button>` : ""}
      ${(item.actions || []).includes("skip") ? `<button class="table-action" data-failure-action="skip" data-automation-failure-action="skip" data-failure-source="${esc(item.source)}" data-failure-id="${esc(item.id)}" type="button">标记跳过</button>` : ""}
      ${(item.actions || []).includes("mark_handled") ? `<button class="table-action" data-failure-action="mark_handled" data-automation-failure-action="mark_handled" data-failure-source="${esc(item.source)}" data-failure-id="${esc(item.id)}" type="button">标记已处理</button>` : ""}
      ${(item.actions || []).includes("manual") ? `<button class="table-action" data-failure-action="manual" data-automation-failure-action="manual" data-failure-source="${esc(item.source)}" data-failure-id="${esc(item.id)}" type="button">转人工</button>` : ""}
...
function renderAutomationLogs() {
  const wrap = $("#automation-log-list");
  if (!wrap) return;
  const rows = (state.automationLogs || []).slice(0, 40).map((item) => `
    <div class="automation-log-row ${item.status || ""}">
      <time>${item.created_at ? new Date(item.created_at * 1000).toLocaleTimeString() : "-"}</time>
      <div><strong>${esc(item.current_step || "-")}</strong><span>${esc(item.message || item.error || "-")}</span></div>
      <small>${esc(item.keyword || "-")} · ${esc(item.product || "-")}</small>
      ${item.screenshot ? `<a href="${esc(item.screenshot)}" target="_blank" rel="noreferrer">截图</a>` : ""}
    </div>
  `).join("");
  wrap.innerHTML = rows || '<div class="table-empty">暂无运行日志。</div>';
}

function renderAutomationFailures() {
  const wrap = $("#automation-failure-list");
  if (!wrap) return;
  wrap.innerHTML = (state.automationFailures || []).slice(0, 20).map(automationFailureCard).join("") || '<div class="table-empty">暂无失败商品。</div>';
  $$('[data-automation-failure-action]', wrap).forEach((button) => button.addEventListener("click", async () => {
    try {
      const action = button.dataset.automationFailureAction;
      ...
      notify(action === "retry" ? "失败任务已进入重试" : "失败任务状态已更新");
...
async function refreshAutomationConsole() {
  const [current, logs, failures] = await Promise.all([
    api("/api/automation/current"),
    api("/api/automation/logs?limit=80"),
    api("/api/automation/failures"),
  ]);
  state.automation = current;
  state.automationLogs = logs.items || [];
  state.automationFailures = failures.items || [];
  renderAutomationConsole();
  renderAutomationLogs();
  renderAutomationFailures();
  return current;
...
async function collectionBulkAction(action) {
  ...
  try {
    let collectedResponse = { items: [], blocked: [] };
    let queueResponse = { created: [], updated: [], blocked: [] };
    if (action === "start" && (payload.candidateIds || []).length) {
      collectedResponse = await api("/api/miaoshou/collect-ready", { method: "POST", body: JSON.stringify({ candidateIds: payload.candidateIds || [] }) });
    }
    if (action !== "start" || (payload.runIds || []).length) {
      queueResponse = await api("/api/collections/bulk-action", { method: "POST", body: JSON.stringify({ action, runIds: payload.runIds || [], candidateIds: action === "start" ? [] : (payload.candidateIds || []) }) });
    }
    const changed = Number((queueResponse.created || []).length + (queueResponse.updated || []).length + (collectedResponse.items || []).length);
    const blocked = Number((queueResponse.blocked || []).length + (collectedResponse.blocked || []).length);
...
function renderPublishResults() {
  ...
  const failureRows = failures.slice(0, 20).map(automationFailureCard).join("") || '<p class="muted-line">暂无失败任务。</p>';
...
function renderMiaoshouRecords() {
  const wrap = $("#miaoshou-box-records");
  if (!wrap) return;
  const rows = (state.miaoshouRecords || []).slice(0, 20).map((item) => {
    const images = item.images_used || item.imagesUsed || [];
    return `<div class="run-row">
      <div><strong>${esc(item.clean_title || item.offer_id || "未命名采集记录")}</strong><span>${esc(item.miaoshou_status || "collected_to_box")} · ${item.collected_at ? new Date(item.collected_at * 1000).toLocaleString() : "-"}</span></div>
      <span class="status-chip">${esc(item.offer_id || "-")}</span>
      <small>${Number(images.length || 0)} 张图 · ${item.source_url ? `<a href="${esc(item.source_url)}" target="_blank" rel="noreferrer">来源</a>` : "无来源"}</small>
    </div>`;
  }).join("");
  wrap.innerHTML = rows || '<div class="table-empty">暂无妙手采集箱记录。</div>';
}
...
async function pollRuntime() {
  ...
  try {
    const [localStatus, browserStatus, platformStatus, sourcing, automation, automationLogs, automationFailures, dashboard, workflow, candidates, products, batches, runs, publishResults, imageJobs, imageSummary, collectionQueue, miaoshouRecords] = await Promise.all([
      api("/api/local/status"), api("/api/browser/status"), api("/api/platform/status"), api("/api/sourcing/current"), api("/api/automation/current"), api("/api/automation/logs?limit=80"), api("/api/automation/failures"), api("/api/dashboard"), api("/api/workflow/summary"), api("/api/candidates"), api("/api/products"), api("/api/batches"), api("/api/runs"), api("/api/publish/results"), api("/api/images/jobs"), api("/api/images/summary"), api(`/api/collections/queue?status=${encodeURIComponent(state.collectionQueueStatus)}`), api("/api/miaoshou/collection-box-records"),
    ]);
...
async function automationAction(action, message) {
  const button = $(`#automation-${action}`);
  if (button) button.disabled = true;
  try {
    const data = await api(`/api/automation/${action}`, { method: "POST", body: "{}" });
...
$("#automation-start").addEventListener("click", () => automationAction("start", "自动找品采集已启动"));
$("#automation-pause").addEventListener("click", () => automationAction("pause", "自动化流程已暂停"));
$("#automation-resume").addEventListener("click", () => automationAction("resume", "自动化流程已继续"));
$("#automation-stop").addEventListener("click", () => automationAction("stop", "自动化流程已停止"));
$("#automation-retry-failed").addEventListener("click", () => automationAction("retry-failed", "失败商品已进入重试"));
$("#automation-refresh-logs").addEventListener("click", async () => { try { await refreshAutomationConsole(); notify("运行日志已刷新"); } catch (error) { notify(error.message, "error"); } });
$("#automation-refresh-failures").addEventListener("click", async () => { try { await refreshAutomationConsole(); notify("失败任务已刷新"); } catch (error) { notify(error.message, "error"); } });
...
$("#collect-qualified").addEventListener("click", async () => {
  try {
    const ids = selectedCandidates();
    const data = await api("/api/miaoshou/collect-ready", { method: "POST", body: JSON.stringify({ candidateIds: ids }) });
    const blocked = data.blocked?.length || 0;
    notify(data.items.length ? `已放入妙手采集箱 ${data.items.length} 个${blocked ? `，拦截 ${blocked} 个候选` : ""}` : "没有符合妙手采集箱条件的候选", data.items.length && !blocked ? "success" : "error");
    await loadAll();
...
$("#collect-pool").addEventListener("click", async () => {
  ...
  try {
    const data = await api("/api/miaoshou/collect-ready", { method: "POST", body: JSON.stringify({ candidateIds: ids }) });
    notify(data.items.length ? `已放入妙手采集箱 ${data.items.length} 个` : "达标池暂时没有可采集商品", data.items.length ? "success" : "error");
    await loadAll();
  } catch (error) {
    notify(error.message, "error");
  }
...
$("#collect-selected-markets").addEventListener("click", async () => {
  ...
  try {
    const data = await api("/api/miaoshou/collect-ready", { method: "POST", body: JSON.stringify({ candidateIds: ids, markets }) });
    notify(data.items.length ? `已按安全采集箱流程处理 ${data.items.length} 个商品` : "没有可采集商品", data.items.length ? "success" : "error");
    await loadAll();
  } catch (error) {
    notify(error.message, "error");
  }
...
loadAll().then(() => window.setInterval(pollRuntime, 2000)).catch((error) => notify(error.message, "error"));
