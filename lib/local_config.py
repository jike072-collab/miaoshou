...
PUBLISH_ACTION_PATTERN = re.compile(
    r"(publish|submit|final\s*publish|confirm\s*publish|发布|上架|最终发布|立即发布|确认发布)",
    re.IGNORECASE,
)
...
def config_status(config):
    return {
        ...
        "allowCollect": bool(config["enable_miaoshou_collect"]) and bool(config["dry_run_collect"]),
